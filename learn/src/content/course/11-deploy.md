...
## Lesson 11.1: Reference deployment (free)
...
GitHub                      ← repo, CI eval gate from Module 09
```
...
## Lesson 11.2: Free-tier realities you must design for

```text
Cold starts / spin-down     free web services sleep after idle → first request slow;
                            long runs must not depend on one warm process (they don't, Module 06)
...
Background execution on a free tier: a FastAPI `BackgroundTasks` or a tiny in-process worker loop is enough for the course. Note explicitly that it is not a real queue: Module 12 will punish that, on purpose.

## Lesson 11.3: Free hosting matrix
...
## Lesson 11.4: Security minimums, even for a demo
...
**Production takeaway:** deployment did not add durability. Modules 03-06 did. Deployment just proved it.
